...
func TestNeutralToolsExpandsToClaudeTools(t *testing.T) {
	...
	result := engine.computeAllowedClaudeToolsString(neutralTools, safeOutputs, cacheMemoryConfig, nil, nil)
...
func TestNeutralToolsWithoutSafeOutputs(t *testing.T) {
	...
	compiler := NewCompiler()
	cacheMemoryConfig, _ := compiler.extractCacheMemoryConfigFromMap(neutralTools)
	result := engine.computeAllowedClaudeToolsString(neutralTools, nil, cacheMemoryConfig, nil, nil)
